...
public class Generator {
    ...
    public static void main(String[] args) throws IOException {
        List<String> slugs = Arrays.asList(sendGetRequest(false,"v4","programs").get("$.items[*].slug").split(",")).stream().map(String::trim).collect(Collectors.toList());
        StringBuffer buffer = new StringBuffer();
        int i = 0;
        for (String slug : slugs) {

            Response r = sendGetRequest(false,"v4", "programs");
